...
func flih_area_attack():
	if player.global_position.x < global_position.x:
		ray_detec_att.target_position = Vector2(-18, 0)
		area_attack.position.x = -16
	else:
		ray_detec_att.target_position = Vector2(18, 0)
		area_attack.position.x = 14

func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player_Minion"):
		body.hit(damage_axe , global_position)
	elif body.is_in_group("Player"):
		body.hit(damage_axe * 2)
